...
void printMask(__cs149_mask v);
// void printVector(__cs149_vec_float v);
...
void printMask(__cs149_mask v){
  for(int i=0;i<VECTOR_WIDTH;i++){printf("%d ",v.value[i]);}
  printf("\n");
}

void printVector(__cs149_vec_float v){
  for(int i=0;i<VECTOR_WIDTH;i++){printf("%f ",v.value[i]);}
  printf("\n");
}

void printVector(__cs149_vec_int v){
  for(int i=0;i<VECTOR_WIDTH;i++){printf("%d",v.value[i]);}
  printf("\n");
}
...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  __cs149_mask compMask,loopMask,clampMask;

  for(int i=0;i<N;i+=VECTOR_WIDTH){
    // computeMask for values index less than N
    compMask = _cs149_init_ones(N-i>VECTOR_WIDTH?VECTOR_WIDTH:N-i);

    // load x and y
    _cs149_vload_float(x,values+i,compMask);
    _cs149_vload_int(y,exponents+i,compMask);
    ...
    printf("y: ");
    printVector(y);
    // loopmask 1 if the loop is still going for that lane
    loopMask = _cs149_init_ones();
    printf("loopMask0: ");
    printMask(loopMask);

    _cs149_veq_int(loopMask, y, zeros, compMask);
    printf("loopMask1: ");
    printMask(loopMask);
    loopMask = _cs149_mask_not(loopMask);

    printf("loopMask2: ");
    printMask(loopMask);
    result = _cs149_vset_float(1.0f);
    int count =0;
    while(_cs149_cntbits(loopMask)!=0 && count<10){
      printf("loopMask3: ");
      printMask(loopMask);
      _cs149_vmult_float(result,result,x,loopMask);
      _cs149_vsub_int(y,y,ones,loopMask);
    ...
    //clamp mask for all >= clamp value
    clampMask = _cs149_init_ones(0);
    _cs149_vgt_float(clampMask, result, clamp, compMask); 
    _cs149_vmove_float(result,clamp,clampMask);
    _cs149_vstore_float(output+i,result,compMask);
  }
}
...
float arraySumVector(float* values, int N) {
  ...
  for (int i=0; i<N; i+=VECTOR_WIDTH) {
    ...
    _cs149_vadd_float(result, result, x, ones);
  }
